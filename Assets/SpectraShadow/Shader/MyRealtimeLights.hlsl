...
Light GetMainLight(float4 shadowCoord, float3 offset, float3 color)
{
    Light light = GetMainLight();
    half4 shadow = MainLightRealtimeShadow(shadowCoord, offset, color);
    light.shadowAttenuation = shadow.a;
    light.color+=shadow.rgb*light.shadowAttenuation;
    return light;
}

Light GetMainLight(float4 shadowCoord, float3 positionWS, half4 shadowMask, float3 offset, float3 color)
{
    Light light = GetMainLight();
...
Light GetMainLight(InputData InputData, half4 shadowMask, AmbientOcclusionFactor aoFactor, float3 offset, float3 color)
{
